...
;	EndIf
	...
	For $x = 0 To $iModeCount - 1
		...
		$iAimTHtext[$x] = "" ;reset
		If $iChkMeetTH[$x] = 1 Then
		   $iAimTHtext[$x] = " [TH]: Level " & StringFormat("%2s", $iMaxTH[$x]) ;$icmbTH
		Else
		   $iAimTHtext[$x] = " [TH]: Any Level" ;$icmbTH
		EndIf
	...
	While 1
		...
		If Mod(($iSkipped + 1), 100) = 0 Then
			...
			If _Sleep(1000) Then Return
;~ 			   Local $icount = 0
;~ 			While Not CheckZoomOut() And $icount < 5
;~ 			   If _Sleep(1000) Then Return
;~ 			   $icount += 1
;~ 			WEnd
;~ 			If $icount = 5 Then Return
		...
		If $OptTrophyMode = 1 Then ;Enables Triple Mode Settings ;---compare resources
			If $THLoc = "Out" Then ; attack this base anyway because outside TH found to snipe
